...
export class BackOfficeController
{
  ...
  async getAllCustomer(@Headers() headers,@Res() res)
  {
    ...
    {
      const dechiperAuth = await this.authService.dechiperUserToken(headers.authorization)
      if(dechiperAuth.userDetail.role != "admin" || dechiperAuth.userDetail.role != "commerciale")
      {
        try{
    ...
    if(isTokenValid)
    {
      ...
      if(dechiperAuth.userDetail.role != "admin" || dechiperAuth.userDetail.role != "commerciale")
      {
        try
        {
          const id = param.id
          console.log({id})
          const foundWorker = await this.userWorkerLoginService.findUserById(id)
          console.log({foundWorker})
          const detail = await this.userWorkerService.findUserDetail(foundWorker.dataValues)
          const role = await this.workerRoleService.findRoleById(detail.id_role)
          const resultDetail = {
            id:detail.id,
  ...
  async modifyUserInfo(@Headers() headers, @Body() body, @Res() res) 
  {
    ...
    if (isTokenValid) {
      const decodedInfo = await this.authService.dechiperUserToken(headers.authorization);
      console.log({ decodedInfo });
      if(decodedInfo.userDetail.role == "servizio clienti")
      {
        const existingRecord = await this.userCustomerService.findOneById(body.id)
